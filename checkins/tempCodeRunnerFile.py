...
def create_checkin(request):
    ...
    # Always fetch latest default questions
    questions = Question.objects.filter(is_default=True)

    if request.method == "POST":

        # ---------------------------------
        # ➕ ADD DEFAULT QUESTION (AJAX – NO PAGE RELOAD)
        # ---------------------------------
        if (
            request.headers.get("x-requested-with") == "XMLHttpRequest"
            and "add_question" in request.POST
        ):
        ...
        # (Triggered ONLY by "Send Check-In")
        # ---------------------------------
        ...
        # ---------------------------------
        # ASSIGN TO EMPLOYEES (ALL or SINGLE)
        # ---------------------------------
